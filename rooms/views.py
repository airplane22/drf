...
class RoomViewSet(ModelViewSet):  # drf viewsets
    queryset = Room.objects.all()
    serializer_class = RoomSerializer
    def get_permissions(self):
        if self.action == "list" or self.action == "retrieve":  # GET rooms/ GET room/#
    ...
    def search(self, request):  # name of action = name of url / change name : url_path(action 가서 확인)
        ...
        if max_price is not None:
            filter_kwargs[
                "price__lte"] = max_price  # django documentation querysets 참조 __lte(less_then, from django)/__gte/__startswith ...
